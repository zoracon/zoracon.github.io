...
1. It seems parameter order may have been updated; APDU errors appeared for me from this guide (I was using standard Ubuntu 18.04). Debugging the APDU errors was elusive. If you have debugging tips, tweet them at me.
2. PIV Manager distribution on Ubuntu in particular is varied. I used the GUI manager because my configuration wasn't playing well with the command-line tools. I had to use an AppImage for Ubuntu 16.04+, rather than the standard download. _shrug_
...
My short guide for code signing with YubiKey is listed below. It's not very extensive on the YubiKey side, because I'm not an expert on the nuances of PIV-compliant smart card configuration and do not want to expand on details if they are inaccurate.
...
1. Once configured, you can generate a public/private key pair.
2. Generate the pair on slot 9c (Digital Signature).
3. Generate the public key and save the output (ex: pub.pem)
   `pkcs15-tool --read-public-key 02`
   - Note: the private key is only accessible through the applet on the smart card.
...
1. Use OpenSSL to generate a hash of the file from its binary form.
2. PIV Tool's `pkcs15-crypt` is called here to sign the hash, reference the key slot to find your YubiKey, and output the signature.
